...
def load_data():
    ...
    with open("zz.json","r",encoding="utf-8") as f:
        str = f.read()
        data = json.loads(str)
    dic_data = dict()
...
def data_viewer():
    ...
    label: 为后面设置legend准备
    """
    rects1 = plt.bar(x=x, height=max, width=0.25, alpha=0.8, color='lightcoral', label="最大值")
    rects2 = plt.bar(x=[i + 0.25 for i in x], height=average, width=0.25, color='orange', label="平均值")
    rects3 = plt.bar(x=[i + 0.5 for i in x], height=min, width=0.25, color='springgreen', label="最小值")
